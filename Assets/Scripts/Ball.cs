...
public class Ball : MonoBehaviour
{
    ...
    [SerializeField] private GameObject newPowerUp;
    [SerializeField] private GameObject powerUpPlataforma;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("block")) // Si colisionamos con un bloque
        {
            ...
            if(random < 0.1f)
            {
                Instantiate(newPowerUp, collision.transform.position, Quaternion.identity);
            }
            if(random < 0.15f)
            {
                //TODO
            }
            Instantiate(powerUpPlataforma, collision.transform.position, Quaternion.identity);
        ...
        if (rb.velocity.x >= velocidadMaxima && rb.velocity.y >= velocidadMaxima)
        {
            rb.velocity = new Vector2(velocidadMaxima, velocidadMaxima);
        }
